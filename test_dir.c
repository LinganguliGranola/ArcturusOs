...
char current_path[PATH_CAPACITY];
char home_path[] = "run/home";

bool filesystem_is_directory(const char *path) { return true; }
bool filesystem_make_directory(const char *path) { return true; }
bool filesystem_create_file(const char *path) { return true; }
void terminal_writestring(const char* s) { printf("%s", s); }
...
bool directory_commands_execute(const char *command) {
    ...
    if (starts_with(command, "cd ") || starts_with(command, "cd")) {
        argument_status = read_optional_argument(command + 2, argument,
                                                 sizeof(argument));
...
int main() {
    ...
    printf("init: >%s<\n", current_path);
    directory_commands_execute("cd ..");
    printf("cd .. -> >%s<\n", current_path);
    directory_commands_execute("cd ..");
    printf("cd .. -> >%s<\n", current_path);
    directory_commands_execute("cd run");
    printf("cd run -> >%s<\n", current_path);
    directory_commands_execute("cd home");
    printf("cd home -> >%s<\n", current_path);
    directory_commands_execute("cd");
    printf("cd -> >%s<\n", current_path);
    directory_commands_execute("cd /");
    printf("cd / -> >%s<\n", current_path);
    directory_commands_execute("cd ~/test");
    printf("cd ~/test -> >%s<\n", current_path);
    
    test_args(); return 0;
}
int test_args() { directory_commands_execute("cd   "); printf("cd -> >%s<\\n", current_path); }
